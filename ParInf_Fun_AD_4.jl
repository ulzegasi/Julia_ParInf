...
@spawnat first_worker ( 
    ...
    firstind = proc_indexes[1][1];
    lastind  = proc_indexes[1][end];

    localpart(V_fast)[1] = quote
        K = T*exp(theta[1])
        g =   exp(theta[2])

        out_fast = ( y[1] - r[1]*exp(u[1]) )^2/(2*sigma^2)
        out_temp = 0.0
        for ii = ($firstind):($lastind)
            out_temp += K * ( u[(ii-1)*j+1] - u[ii*j+1] )^2/( j*dt ) / ( 2*g ) +
                                ( y[ii+1] - r[ii*j+1]*exp(u[ii*j+1]) )^2/(2*sigma^2)
        end
        for ii = ($firstind):($lastind)
            for k = 2:j
                out_temp += k*u[(ii-1)*j+k]^2/( (k-1)*dt )*K/(2*g)
            end
        end
        out_fast += out_temp
    end
...
for i = 2:num_workers
    @spawnat workers()[i] (
        firstind = proc_indexes[i][1];
        lastind  = proc_indexes[i][end];
        localpart(V_fast)[1] = quote
            K = T*exp(theta[1])
            g =   exp(theta[2])
...
for i = 1:num_workers
    @spawnat workers()[i] (
        ...
        localpart(V_fast_der_u_expr)[1] = rdiff(localpart(V_fast)[1], outsym=:out_fast, u     = ones(Float64, N));
    )
end
...
@spawnat last_worker (
    ...
    firstind = proc_indexes[end][1];
    lastind  = proc_indexes[end][end];
