...
dir3  = "C:/Users/ulzegasi/SWITCHdrive/JuliaTemp/Orders"
...
nsample_burnin  = 2000                        # Number of system realizations (burnin and effective) 
nsample_eff     = 10
...
# Initialization of vectors to store values of different parts of the Hamiltonian
vfast_bdy  = zeros(nsample_eff)
vfast_stg  = zeros(nsample_eff)
vfast_dat  = zeros(nsample_eff)
vfast_par  = zeros(nsample_eff)
vslow_sum  = zeros(nsample_eff)
vslow_cnt  = zeros(nsample_eff) 
...
for counter = (nsample_burnin + 1):nsample
    ...
    local_counter = counter - nsample_burnin
    vfast_dat[local_counter] = ( y[1] - r[1]*exp(u[1]) )^2/(2*sigma^2)
    for i=1:n
        vfast_bdy[local_counter] += K * ( u[(i-1)*j+1] - u[i*j+1] )^2/( j*dt ) / (2*gamma)
        vfast_dat[local_counter] += ( y[i+1] - r[i*j+1]*exp(u[i*j+1]) )^2/(2*sigma^2)
        for k=2:j
            vfast_stg[local_counter] += k*u[(i-1)*j+k]^2/( (k-1)*dt ) * K/(2*gamma)
        end
    end

    vfast_par[local_counter] = (N/2.0) * (theta[1]-theta[2])

    for i = 1:(n-1)
        vslow_sum[local_counter] += dt * ( (K * lnr_der[i*j+1] + gamma + 1) - exp( -u[i*j+1] ) )^2 / (2.0*gamma*K) -
    ...
    for i = 1:n
        for k = 2:j
            ...
            vslow_sum[local_counter] += dt * ( (K * lnr_der[(i-1)*j+k] + gamma + 1) - exp( -tmp ) )^2 / (2.0*gamma*K) -
                                        dt * tmp * ( K*(lnr_der[(i-1)*j+k]-lnr_der[(i-1)*j+k-1])/dt ) / gamma
        end
    end

    vslow_cnt[local_counter] = ( exp(- u[N]) + u[N]*(K * lnr_der[N-1] + gamma + 1) ) / gamma -
...
## ============================================================================================
##
fname = string("_N$N")   # Output files
...
writedlm("$dir3/vfast_bdy$fname.dat", vfast_bdy)
writedlm("$dir3/vfast_stg$fname.dat", vfast_stg)
writedlm("$dir3/vfast_dat$fname.dat", vfast_dat)
writedlm("$dir3/vfast_par$fname.dat", vfast_par)
writedlm("$dir3/vslow_sum$fname.dat", vslow_sum)
writedlm("$dir3/vslow_cnt$fname.dat", vslow_cnt)